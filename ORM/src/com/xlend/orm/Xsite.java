...
public class Xsite extends DbObject  {
    ...
    private Integer xMap = null;
    private Integer yMap = null;

    public Xsite(Connection connection) {
        super(connection, "xsite", "xsite_id");
        setColumnNames(new String[]{"xsite_id", "name", "description", "dieselsponsor", "sitetype", "xorder_id", "xorder2_id", "xorder3_id", "is_active", "x_map", "y_map"});
    }

    public Xsite(Connection connection, Integer xsiteId, String name, String description, Integer dieselsponsor, String sitetype, Integer xorderId, Integer xorder2Id, Integer xorder3Id, Integer isActive, Integer xMap, Integer yMap) {
        super(connection, "xsite", "xsite_id");
...
//        }
    ...
    public DbObject loadOnId(int id) throws SQLException, ForeignKeyViolationException {
        ...
        ResultSet rs = null;
        String stmt = "SELECT xsite_id,name,description,dieselsponsor,sitetype,xorder_id,xorder2_id,xorder3_id,is_active,x_map,y_map FROM xsite WHERE xsite_id=" + id;
        try {
            ...
            if (rs.next()) {
                ...
                xsite.setXMap(new Integer(rs.getInt(10)));
                xsite.setYMap(new Integer(rs.getInt(11)));
                xsite.setNew(false);
            }
        } finally {
            try {
    ...
    protected void insert() throws SQLException, ForeignKeyViolationException {
         ...
         PreparedStatement ps = null;
         String stmt =
                "INSERT INTO xsite ("+(getXsiteId().intValue()!=0?"xsite_id,":"")+"name,description,dieselsponsor,sitetype,xorder_id,xorder2_id,xorder3_id,is_active,x_map,y_map) values("+(getXsiteId().intValue()!=0?"?,":"")+"?,?,?,?,?,?,?,?,?,?)";
         try {
             ps = getConnection().prepareStatement(stmt);
             int n = 0;
             ...
             ps.setObject(++n, getXMap());
             ps.setObject(++n, getYMap());
             ps.execute();
         } finally {
             if (ps != null) ps.close();
         }
    ...
    public void save() throws SQLException, ForeignKeyViolationException {
        if (isNew()) {
            ...
            String stmt =
                    "UPDATE xsite " +
                    "SET name = ?, description = ?, dieselsponsor = ?, sitetype = ?, xorder_id = ?, xorder2_id = ?, xorder3_id = ?, is_active = ?, x_map = ?, y_map = ?" + 
            ...
            try {
                ...
                ps.setObject(9, getXMap());
                ps.setObject(10, getYMap());
                ps.execute();
            } finally {
                if (ps != null) ps.close();
            }
    ...
    public void delete() throws SQLException, ForeignKeyViolationException {
        ...
        if (Xaccpayment.exists(getConnection(),"xsite_id = " + getXsiteId())) {
            throw new ForeignKeyViolationException("Can't delete, foreign key violation: xaccpayment_xsite_fk");
        }
        if (Xmoveableassets.exists(getConnection(),"xsite_id = " + getXsiteId())) {
            throw new ForeignKeyViolationException("Can't delete, foreign key violation: xmoveableassets_xsite_fk");
        }
        ...
        if (Xmachineorder.exists(getConnection(),"xsite_id = " + getXsiteId())) {
            throw new ForeignKeyViolationException("Can't delete, foreign key violation: xmachineorder_xsite_fk");
        }
        ...
        if (Xpettyitem.exists(getConnection(),"xsite_id = " + getXsiteId())) {
            throw new ForeignKeyViolationException("Can't delete, foreign key violation: xpettyitem_xsite_fk");
        }
    ...
    public static DbObject[] load(Connection con,String whereCondition,String orderCondition) throws SQLException {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String stmt = "SELECT xsite_id,name,description,dieselsponsor,sitetype,xorder_id,xorder2_id,xorder3_id,is_active,x_map,y_map FROM xsite " +
        ...
        try {
            ...
            while (rs.next()) {
                DbObject dbObj;
                lst.add(dbObj=new Xsite(con,new Integer(rs.getInt(1)),rs.getString(2),rs.getString(3),new Integer(rs.getInt(4)),rs.getString(5),new Integer(rs.getInt(6)),new Integer(rs.getInt(7)),new Integer(rs.getInt(8)),new Integer(rs.getInt(9)),new Integer(rs.getInt(10)),new Integer(rs.getInt(11))));
                dbObj.setNew(false);
            }
        } finally {
            try {
    ...
    public void setXMap(Integer xMap) throws SQLException, ForeignKeyViolationException {
        if (null != xMap)
            xMap = xMap == 0 ? null : xMap;
        setWasChanged(this.xMap != null && !this.xMap.equals(xMap));
        this.xMap = xMap;
    }
    ...
    public void setYMap(Integer yMap) throws SQLException, ForeignKeyViolationException {
        if (null != yMap)
            yMap = yMap == 0 ? null : yMap;
        setWasChanged(this.yMap != null && !this.yMap.equals(yMap));
        this.yMap = yMap;
    }
    public Object[] getAsRow() {
        ...
        columnValues[9] = getXMap();
        columnValues[10] = getYMap();
        return columnValues;
    }
    ...
    public void fillFromString(String row) throws ForeignKeyViolationException, SQLException {
        ...
        try {
            setXMap(Integer.parseInt(flds[9]));
        } catch(NumberFormatException ne) {
            setXMap(null);
        }
        try {
            setYMap(Integer.parseInt(flds[10]));
        } catch(NumberFormatException ne) {
            setYMap(null);
        }
    }
