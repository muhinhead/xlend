...
public class Xsupplier extends DbObject  {
    ...
    private Integer isFuelSuppllier = null;

    public Xsupplier(Connection connection) {
        super(connection, "xsupplier", "xsupplier_id");
        setColumnNames(new String[]{"xsupplier_id", "companyname", "contactperson", "productdesc", "phone", "fax", "cell", "email", "vatnr", "company_regnr", "address", "banking", "credit_limit", "is_fuel_suppllier"});
    }

    public Xsupplier(Connection connection, Integer xsupplierId, String companyname, String contactperson, String productdesc, String phone, String fax, String cell, String email, String vatnr, String companyRegnr, String address, String banking, Integer creditLimit, Integer isFuelSuppllier) {
        super(connection, "xsupplier", "xsupplier_id");
...
//        }
        ...
        this.isFuelSuppllier = isFuelSuppllier;
    }
    ...
    public DbObject loadOnId(int id) throws SQLException, ForeignKeyViolationException {
        ...
        ResultSet rs = null;
        String stmt = "SELECT xsupplier_id,companyname,contactperson,productdesc,phone,fax,cell,email,vatnr,company_regnr,address,banking,credit_limit,is_fuel_suppllier FROM xsupplier WHERE xsupplier_id=" + id;
        try {
            ...
            if (rs.next()) {
                ...
                xsupplier.setCreditLimit(new Integer(rs.getInt(13)));
                xsupplier.setIsFuelSuppllier(new Integer(rs.getInt(14)));
    ...
    protected void insert() throws SQLException, ForeignKeyViolationException {
         ...
         PreparedStatement ps = null;
         String stmt =
                "INSERT INTO xsupplier ("+(getXsupplierId().intValue()!=0?"xsupplier_id,":"")+"companyname,contactperson,productdesc,phone,fax,cell,email,vatnr,company_regnr,address,banking,credit_limit,is_fuel_suppllier) values("+(getXsupplierId().intValue()!=0?"?,":"")+"?,?,?,?,?,?,?,?,?,?,?,?,?)";
         try {
             ps = getConnection().prepareStatement(stmt);
             int n = 0;
             ...
             ps.setObject(++n, getCreditLimit());
             ps.setObject(++n, getIsFuelSuppllier());
             ps.execute();
         } finally {
             if (ps != null) ps.close();
         }
    ...
    public void save() throws SQLException, ForeignKeyViolationException {
        if (isNew()) {
            ...
            String stmt =
                    "UPDATE xsupplier " +
                    "SET companyname = ?, contactperson = ?, productdesc = ?, phone = ?, fax = ?, cell = ?, email = ?, vatnr = ?, company_regnr = ?, address = ?, banking = ?, credit_limit = ?, is_fuel_suppllier = ?" + 
            ...
            try {
                ...
                ps.setObject(12, getCreditLimit());
                ps.setObject(13, getIsFuelSuppllier());
                ps.execute();
            } finally {
                if (ps != null) ps.close();
            }
    ...
    public void delete() throws SQLException, ForeignKeyViolationException {
        ...
        if (Xbatterypurchase.exists(getConnection(),"xsupplier_id = " + getXsupplierId())) {
            throw new ForeignKeyViolationException("Can't delete, foreign key violation: xbatterypurchase_xsupplier_fk");
        }
    ...
    public static DbObject[] load(Connection con,String whereCondition,String orderCondition) throws SQLException {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String stmt = "SELECT xsupplier_id,companyname,contactperson,productdesc,phone,fax,cell,email,vatnr,company_regnr,address,banking,credit_limit,is_fuel_suppllier FROM xsupplier " +
        ...
        try {
            ...
            while (rs.next()) {
                DbObject dbObj;
                lst.add(dbObj=new Xsupplier(con,new Integer(rs.getInt(1)),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9),rs.getString(10),rs.getString(11),rs.getString(12),new Integer(rs.getInt(13)),new Integer(rs.getInt(14))));
                dbObj.setNew(false);
            }
        } finally {
            try {
    ...
    public void setCreditLimit(Integer creditLimit) throws SQLException, ForeignKeyViolationException {
        if (null != creditLimit)
            creditLimit = creditLimit == 0 ? null : creditLimit;
        setWasChanged(this.creditLimit != null && !this.creditLimit.equals(creditLimit));
        this.creditLimit = creditLimit;
    }

    public Integer getIsFuelSuppllier() {
        return isFuelSuppllier;
    }

    public void setIsFuelSuppllier(Integer isFuelSuppllier) throws SQLException, ForeignKeyViolationException {
        if (null != isFuelSuppllier)
            isFuelSuppllier = isFuelSuppllier == 0 ? null : isFuelSuppllier;
        setWasChanged(this.isFuelSuppllier != null && !this.isFuelSuppllier.equals(isFuelSuppllier));
        this.isFuelSuppllier = isFuelSuppllier;
    }
    public Object[] getAsRow() {
        ...
        columnValues[12] = getCreditLimit();
        columnValues[13] = getIsFuelSuppllier();
        return columnValues;
    }
    ...
    public void fillFromString(String row) throws ForeignKeyViolationException, SQLException {
        ...
        setBanking(flds[11]);
        try {
            setCreditLimit(Integer.parseInt(flds[12]));
        } catch(NumberFormatException ne) {
            setCreditLimit(null);
        }
        try {
            setIsFuelSuppllier(Integer.parseInt(flds[13]));
        } catch(NumberFormatException ne) {
            setIsFuelSuppllier(null);
        }
    }
}
