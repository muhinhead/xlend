...
public class Xpetty extends DbObject  {
    ...
    public Xpetty(Connection connection) {
        ...
        setColumnNames(new String[]{"xpetty_id", "issue_date", "xemployee_in_id", "amount", "change_amt", "is_loan", "is_petty", "is_allowance", "notes", "receipt_date", "xemployee_out_id"});
    }

    public Xpetty(Connection connection, Integer xpettyId, Date issueDate, Integer xemployeeInId, Double amount, Double changeAmt, Integer isLoan, Integer isPetty, Integer isAllowance, String notes, Date receiptDate, Integer xemployeeOutId) {
        super(connection, "xpetty", "xpetty_id");
...
//        }
    ...
    public DbObject loadOnId(int id) throws SQLException, ForeignKeyViolationException {
        ...
        ResultSet rs = null;
        String stmt = "SELECT xpetty_id,issue_date,xemployee_in_id,amount,change_amt,is_loan,is_petty,is_allowance,notes,receipt_date,xemployee_out_id FROM xpetty WHERE xpetty_id=" + id;
        try {
            ...
            if (rs.next()) {
                ...
                xpetty.setIsLoan(new Integer(rs.getInt(6)));
                xpetty.setIsPetty(new Integer(rs.getInt(7)));
                xpetty.setIsAllowance(new Integer(rs.getInt(8)));
                xpetty.setNotes(rs.getString(9));
                xpetty.setReceiptDate(rs.getDate(10));
                xpetty.setXemployeeOutId(new Integer(rs.getInt(11)));
                xpetty.setNew(false);
            }
        } finally {
            try {
    ...
    protected void insert() throws SQLException, ForeignKeyViolationException {
         ...
         PreparedStatement ps = null;
         String stmt =
                "INSERT INTO xpetty ("+(getXpettyId().intValue()!=0?"xpetty_id,":"")+"issue_date,xemployee_in_id,amount,change_amt,is_loan,is_petty,is_allowance,notes,receipt_date,xemployee_out_id) values("+(getXpettyId().intValue()!=0?"?,":"")+"?,?,?,?,?,?,?,?,?,?)";
         try {
             ps = getConnection().prepareStatement(stmt);
             int n = 0;
    ...
    public void save() throws SQLException, ForeignKeyViolationException {
        if (isNew()) {
            ...
            String stmt =
                    "UPDATE xpetty " +
                    "SET issue_date = ?, xemployee_in_id = ?, amount = ?, change_amt = ?, is_loan = ?, is_petty = ?, is_allowance = ?, notes = ?, receipt_date = ?, xemployee_out_id = ?" + 
            ...
            try {
                ...
                ps.setObject(5, getIsLoan());
                ps.setObject(6, getIsPetty());
                ps.setObject(7, getIsAllowance());
                ps.setObject(8, getNotes());
                ps.setObject(9, getReceiptDate());
                ps.setObject(10, getXemployeeOutId());
                ps.execute();
            } finally {
                if (ps != null) ps.close();
            }
    ...
    public static DbObject[] load(Connection con,String whereCondition,String orderCondition) throws SQLException {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String stmt = "SELECT xpetty_id,issue_date,xemployee_in_id,amount,change_amt,is_loan,is_petty,is_allowance,notes,receipt_date,xemployee_out_id FROM xpetty " +
        ...
        try {
            ...
            while (rs.next()) {
                DbObject dbObj;
                lst.add(dbObj=new Xpetty(con,new Integer(rs.getInt(1)),rs.getDate(2),new Integer(rs.getInt(3)),rs.getDouble(4),rs.getDouble(5),new Integer(rs.getInt(6)),new Integer(rs.getInt(7)),new Integer(rs.getInt(8)),rs.getString(9),rs.getDate(10),new Integer(rs.getInt(11))));
                dbObj.setNew(false);
            }
        } finally {
            try {
    ...
    public Object[] getAsRow() {
        ...
        columnValues[5] = getIsLoan();
        columnValues[6] = getIsPetty();
        columnValues[7] = getIsAllowance();
        columnValues[8] = getNotes();
        columnValues[9] = getReceiptDate();
        columnValues[10] = getXemployeeOutId();
        return columnValues;
    }
    ...
    public void fillFromString(String row) throws ForeignKeyViolationException, SQLException {
        ...
        try {
            setIsLoan(Integer.parseInt(flds[5]));
        } catch(NumberFormatException ne) {
            setIsLoan(null);
        }
        try {
            setIsPetty(Integer.parseInt(flds[6]));
        } catch(NumberFormatException ne) {
            setIsPetty(null);
        }
        try {
            setIsAllowance(Integer.parseInt(flds[7]));
        } catch(NumberFormatException ne) {
            setIsAllowance(null);
        }
        setNotes(flds[8]);
        setReceiptDate(toDate(flds[9]));
        try {
            setXemployeeOutId(Integer.parseInt(flds[10]));
        } catch(NumberFormatException ne) {
